...
#          print("  %5i %5i %8.5f %8.5f %s"%(n+1, len(evals), 27.21138*evals[n],de[n],("Y" if (self.is_converged[n] == 1) else "N")),flush=True)
    ...
    def sigma(self, Xt):
        ...
        for n in range(ndim):
          ...
          Darray[2] = D[:nbf,nbf:]
          Darray[3] = D[nbf:,:nbf]
          j, k = self.jk.get_jk(mol=self.ints,dm=Darray,hermi=0)
    ...
    def __init__(self, nel, eps_a, Cmo,ints_factory,jk):
        self.ints_factory = ints_factory
        self.jk = jk
        self.nbf   = len(eps_a)
        self.no_a  = nel
        self.nv_a  = int(self.nbf - self.no_a)
...
class RGHESSIAN():
    ...
    #    print("    TIME: One-body contribution to sigma: %f"%(time.time()-tstart),flush=True)
        ...
        if self.in_core is False:
            ...
            for n in range(ndim):
              tmp = np.einsum("ma,ai->mi",self.Cv,Xt[:,:,n],optimize=True)
              D   = np.einsum("mi,ni->mn",tmp,self.Co,optimize=True)
              Darray[0] = D[:nbf,:nbf]
              Darray[1] = D[nbf:,nbf:]
              Darray[2] = D[:nbf,nbf:]
              Darray[3] = D[nbf:,:nbf]
...
def transform(g,C,nmo,nel):

    #J_IAJB
    J_iajb  = j_contract(g,C[:nmo,:nel].conj(),C[:nmo,nel:],C[:nmo,:nel].conj(),C[:nmo,nel:])
    J_iajb += j_contract(g,C[nmo:,:nel].conj(),C[nmo:,nel:],C[:nmo,:nel].conj(),C[:nmo,nel:])
    J_iajb += j_contract(g,C[nmo:,:nel].conj(),C[nmo:,nel:],C[nmo:,:nel].conj(),C[nmo:,nel:])
    J_iajb += j_contract(g,C[:nmo,:nel].conj(),C[:nmo,nel:],C[nmo:,:nel].conj(),C[nmo:,nel:])

    K_iajb  = k_contract(g,C[:nmo,:nel].conj(),C[:nmo,nel:],C[:nmo,:nel].conj(),C[:nmo,nel:])
    K_iajb += k_contract(g,C[nmo:,:nel].conj(),C[nmo:,nel:],C[nmo:,:nel].conj(),C[nmo:,nel:])

    J_iabj  = j_contract(g,C[:nmo,:nel].conj(),C[:nmo,nel:],C[:nmo,nel:].conj(),C[:nmo,:nel])
    J_iabj += j_contract(g,C[nmo:,:nel].conj(),C[nmo:,nel:],C[:nmo,nel:].conj(),C[:nmo,:nel])
    J_iabj += j_contract(g,C[nmo:,:nel].conj(),C[nmo:,nel:],C[nmo:,nel:].conj(),C[nmo:,:nel])
    J_iabj += j_contract(g,C[:nmo,:nel].conj(),C[:nmo,nel:],C[nmo:,nel:].conj(),C[nmo:,:nel])

    K_iabj  = k_contract(g,C[:nmo,:nel].conj(),C[:nmo,nel:],C[:nmo,nel:].conj(),C[:nmo,:nel])
    K_iabj += k_contract(g,C[nmo:,:nel].conj(),C[nmo:,nel:],C[nmo:,nel:].conj(),C[nmo:,:nel])

    return J_iajb, K_iajb, J_iabj, K_iabj

def j_contract(g,c1,c2,c3,c4):
   I_pNLS = np.einsum("MNLS,Mp->pNLS",g,c1,optimize=True)
   I_pqLS = np.einsum("pNLS,Nq->pqLS",I_pNLS,c2,optimize=True)
   I_pqrS = np.einsum("pqLS,Lr->pqrS",I_pqLS,c3,optimize=True)
   J_pqrs = np.einsum("pqrS,Ss->pqrs",I_pqrS,c4,optimize=True)

   return J_pqrs

def k_contract(g,c1,c2,c3,c4):
   I_pSLN = np.einsum("MSLN,Mp->pSLN",g,c1,optimize=True)
   I_pSLq = np.einsum("pSLN,Nq->pSLq",I_pSLN,c2,optimize=True)
   I_pSrq = np.einsum("pSLq,Lr->pSrq",I_pSLq,c3,optimize=True)
   K_psrq = np.einsum("pSrq,Ss->psrq",I_pSrq,c4,optimize=True)
